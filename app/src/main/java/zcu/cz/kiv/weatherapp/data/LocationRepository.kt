...
class LocationRepository(context: Context) {
    ...
    suspend fun search(name: String): List<GeoLocationDto> =
        api.search(name)

    suspend fun getFavorites(): List<FavoriteLocationResponse> =
        api.favorites(authHeader())

    suspend fun addFavorite(loc: GeoLocationDto): FavoriteLocationResponse =
        api.addFavorite(
            authHeader(),
    ...
    suspend fun deleteFavorite(id: String) {
        api.deleteFavorite(authHeader(), id)
    }
}